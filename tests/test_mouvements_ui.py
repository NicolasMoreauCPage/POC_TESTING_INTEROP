...
def test_edit_mouvement_page_renders(client, session: Session):
    ...
    d = Dossier(dossier_seq=1, patient_id=p.id, uf_medicale="UF-TEST",
 uf_hebergement="UF-TEST", admit_time=datetime.now(), dossier_type=DossierType.HOSPITALISE)
    session.add(d); session.commit(); session.refresh(d)

    v = Venue(venue_seq=1, dossier_id=d.id, uf_medicale="UF-TEST",
 uf_hebergement="UF-TEST", start_time=datetime.now())
